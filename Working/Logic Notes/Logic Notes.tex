...
Suppose $\mc{A}$ is a permissive formula. If $\mc{A}$ is atomic then $\mc{A}[0]$ is in \textbf{Pred}. If $\mc{A}$ is an implication, negation, or quantification, then $\mc{A}[0] \equiv ($. So for every permissive formula $\mc{A}[0]$ is in $\textbf{Pred}$ or is equal to $($.

Now suppose $\mc{A}$ is a permissive formula. If $\mc{A}$ is atomIf $\mc{A}$ is an implication then $\mc{A}\equiv (\mc{B}\implies \mc{C})$. $\mc{A}[1] \equiv \mc{B}[0]$ so $\mc{A}[1]$ is either an element of $\textbf{Pred}$ or equal to $($. If $\mc{A}$ is a negation then $\mc{A}[1] \equiv \lnot$. If $\mc{A}$ is a quantification then $\mc{A} \equiv ($. This means, for non-atomic $\mc{A}$, that $\mc{A}[1]$ is either an element of $\textbf{Pred}$, equal to $($, or equal to $\lnot$.

}}


\noindent\fbox{\parbox{\textwidth}{ 
\textbf{Unique Range of Quantification Rules}

Here we will show that if $\mc{A}$ is a permissive formula then it is exactly one of an atomic formula, an implication, a negation, or a quantification, but not more than one.

Suppose $\mc{A}$ is an atomic formula. Then $\mc{A}[0] \in \textbf{Pred}$. 
If $\mc{A}$ was also either an implication, negation, or quantification we would have $\mc{A}[0] \equiv ( \not \in \textbf{Pred}$ so $\mc{A}$ is not an implication, negation, or quantification.

Suppose $\mc{A}$ is an implication so $\mc{A}\equiv(\mc{B} \implies \mc{C})$ for $\mc{B},\mc{C} \in \textbf{pForm}$.
We know from above that $\mc{A}$ is not atomic. 
If $\mc{A}$ was a negation then we would have $\mc{A}[1] \equiv \lnot$.
But $\mc{A}[1] \equiv \mc{B}[0]$, but we have from the theorem above that $\mc{B}[0]$ can only be in $\textbf{Pred}$ or equal to $($, so $\mc{A}$ is not a negation.
If $\mc{A}$ was a quantification we would have $\mc{A}[2] \equiv \forall$.
If $|\mc{B}|=1$ then $\mc{A}[2]\equiv \implies \not \equiv \forall$.
If $|\mc{B}|>1$ then $\mc{A}[2] \equiv \mc{B}[1]$, but we have from the theorem above that $\mc{B}[1]$ can only be in $\textbf{Pred}$, equal to $($, or equal to $\lnot$ so $\mc{A}$ is not a quantificaiton.

Suppose $\mc{A}$ is a negation so $\mc{A} \equiv (\lnot \mc{B})$ for $\mc{B}\in\textbf{pForm}$. 
We know from above that $\mc{A}$ is not atomic or an implication.
If $\mc{A}$ was a quantification then we would have $\mc{A}[2] \equiv \forall$.
But $\mc{A}[2] \equiv \mc{B}[1]$, but we have from the theorem above that $\mc{B}[1]$ can only be in $\textbf{Pred}$, equal to $($ or equal to $\lnot$ so $\mc{A}$ is not a quantification.
An alternative proof would be to notice that if $\mc{A}$ was a quantification we would have $\mc{A}[1] \equiv ($ but $\mc{A}[1]$ is in fact equal to $\lnot$.

}}


\noindent\fbox{\parbox{\textwidth}{ 
\textbf{No Proper Initial Segment of a Permissive Formula is a Permissive Formula}

suppose $\mc{A}$ is a permissive formula.

Suppose $\mc{A}$ is atomic. Suppose that $\mc{A}'$ is a proper intial substring of $\mc{A}$ and suppose for contraction that $\mc{A}'$ is a permissive formula. Since $\mc{A}$ is atomic we have $\mc{A}[0] \equiv \mc{A}'[0] \equiv P$. Thus $\mc{A}$ must also be atomic. This is because if $\mc{A}'$ were a negation, implication, or quantification then we would have had $\mc{A}'[0] \equiv ( \not \equiv P$. This means $\mc{A}' \equiv P t_1'\ldots t_n'$. But, by a previous theorem, we must have  that either $t_1$ is an initial substring of $t_1'$ or $t_1'$ is an initial substring of $t_1$, but because terms cannot have proper initial substrings this means $t_1 \equiv t_1'$. This argument can be repeated up through $t_n$ and $t_n'$ showing that $\mc{P}' \equiv \mc{P}$ and $t_k'\equiv t_k$ for $1\le k \le n$ meaning that $\mc{A} \equiv \mc{A}'$, contradicting the assumptions that $\mc{A}'$ is a proper initial substring of $\mc{A}$.

Now suppose $\mc{A}$ is a negation so $\mc{A}\equiv (\lnot\mc{B})$. Suppose that $\mc{A}'$ is a proper initial substring of $\mc{A}$ and, for contradiction, that $\mc{A}'$ is a permissive formula. $\mc{A}'$ cannot be atomic because $\mc{A}'[0] \equiv \mc{A}[0] \equiv ( \not \in \textbf{Pred}$. Suppose that $\mc{A}'$ is an implication. $\mc{A}' \equiv (\mc{B}'\implies \mc{C}')$. It can be shown (by the definition of concatenation) and $\mc{B}'$ being non-empty that $\mc{A}'[1] \equiv \mc{B}'[0]$. But $\mc{A}'[1] \equiv \mc{A}[1] \equiv \lnot$ which would imply $\mc{B}'[0] \equiv \lnot$, but there is no permissive formula which begins with $\lnot$. Similarly, 

Note that $\mc{A}'[1] \equiv \mc{A}[0]$
